...
@test all(Proportion(Radian(Degree(Radian(Proportion(x))))).x ≈ x for x in -10:rand():10)

@test all(Degree(x) == x for x in Degree.(-10:rand():10))
@test all(Radian(x) == x for x in Radian.(-10:rand():10))
@test all(Proportion(x) == x for x in Proportion.(-10:rand():10))

for fun in (:sin, :sinc, :cos, :cosc, :tan, :sec, :csc, :cot), degree in -89:50rand():89
    radian = deg2rad(degree)
    proportion = degree/180
    goal = @eval $fun($radian)
    @test @eval $fun(Degree($degree)) ≈ $fun(Radian($radian)) ≈ $fun(Proportion($proportion)) ≈ $goal
end
...
for fun in (:tan, :cot), x in -10:2rand():10
    @test @eval $(Symbol("a$(fun)"))(Proportion, $x).x ≈ $(Symbol("a$(fun)"))($x)/π
end
